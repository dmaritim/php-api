...
private $conn;
private $table_name = "marriage_type";
// object properties
public $marriage_type_id;
public $name;
...
public function getSingleStewardship(){
  ...
  $this->marriage_type_id=htmlspecialchars(strip_tags($this->marriage_type_id));

  $stmt = $this->conn->prepare($sqlQuery);

  $stmt->bindParam(1, $this->marriage_type_id);
...
function deleteRelationship(){
  $sqlQuery = "DELETE FROM " . $this->table_name . " WHERE marriage_type_id = ?";
  $stmt = $this->conn->prepare($sqlQuery);
...
public function read($page=1,$pagesize=5){
    ...
    if($number_of_results){
      ...
      $query = 'SELECT marriage_type_id, name
      FROM ' . $this->table_name . ' p
